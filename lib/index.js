  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      if (self.options.isFeathersJS) {
        ...
        text[table] += spaces + spaces + spaces + "beforeCount(options) {\n";
        text[table] += spaces + spaces + spaces + spaces + "options.raw = true;\n";
        text[table] += spaces + spaces + spaces + "}\n";
        text[table] += spaces + spaces + "}\n";
        text[table] += spaces + "}\n";
      }
      ...
      text[table] += ");\n\n";
      text[table] += spaces + "// eslint-disable-next-line no-unused-vars\n";
      text[table] += spaces + tableName + ".associate = function (models) {\n"
      text[table] += spaces + spaces + "// Define associations here\n";
      text[table] += spaces + spaces + "// See https://sequelize.org/master/manual/assocs.html\n";
      text[table] += spaces + "};\n\n";
